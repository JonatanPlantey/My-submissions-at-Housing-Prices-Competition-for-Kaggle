...
pd.set_option('display.max_columns', 27, 'display.max_colwidth', 11,
...
def normalize(sum_, length, min_, max_):
    return round( ( ( ( (sum_/length) ** 6 / np.sqrt(max_) ) * 1 ) ))

def average_for_value_in_prices_normalized(df, value):
    df_copy = df.copy()
    df_copy.drop( df_copy[ df_copy.iloc[:, 0] != value ].index, inplace=True)
    sum_ = sum(df_copy.iloc[:, 1])
    length = len(df_copy.iloc[:, 1])
    min_ = df_copy.iloc[:, 1].min()
    max_ = df_copy.iloc[:, 1].max()
    if length > 0:
        return( normalize(sum_, length, min_, max_) )
    else:
        return(0)

def average_for_NaN_in_prices_normalized(df):
    df_copy = df.copy()
...
for feature in features_to_convert:
    ...
    dict_feature = {feature_value_counts.index[i] : average_for_value_in_prices_normalized(feature_and_prices, feature_value_counts.index[i])
                    for i in range(len(feature_value_counts))}
    dict_feature["Unknown"] = average_for_NaN_in_prices_normalized(feature_and_prices)
